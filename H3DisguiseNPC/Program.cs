	...
	private static async void Mod()
	{
		...
		case -1:
		{
			...
			if (GlobalOutfitKit.PointerAddressValid(num3))
			{
				...
				{
					Console.WriteLine("\nGlobalOutfitKit Ptr Address: 0x" + num3.ToString("X2") + "\nGlobalOutfitKit Ptr: " + gokPtr);
					currentStatus++;
				}
			}
			else
		...
		case 0:
		{
			IEnumerable<long> gokAddresses = await mem.AoBScan(536870912L, 805306368L, BitConverter.GetBytes(gokPtr).ToLiteralByteArrayString(), writable: true, executable: false);
			ovcPtr = mem.readLong((gokAddresses.LastOrDefault() + 152).ToString("X2") + ",0x00");
			...
			foreach (long item in gokAddresses)
			{
				...
				Console.WriteLine("GlobalOutfitKit\n{\n\tAddress: 0x" + globalOutfitKit.Address.ToString("X2") + "\n\tCommonName: " + globalOutfitKit.CommonName + "\n\tName: " + globalOutfitKit.Name + "\n\tHeroDisguiseAvailable: " + globalOutfitKit.HeroDisguiseAvailable + "\n}");
				if (!globalOutfitKit.HeroDisguiseAvailable)
				{
					Console.WriteLine("\nEnabling Hero Disguise Availability for: " + globalOutfitKit.CommonName + "\n");
					globalOutfitKit.EnableHeroDisguiseAvailability();
				}
			}
			IEnumerable<long> enumerable = await mem.AoBScan(536870912L, 805306368L, BitConverter.GetBytes(ovcPtr).ToLiteralByteArrayString(), writable: true, executable: false);
			foreach (long item2 in enumerable)
			{
				Console.WriteLine("OutfitVariationCollection\n{\n\tAddress: 0x" + item2.ToString("X2") + "\n}");
			...
			Console.WriteLine("Finished modifying " + gokAddresses.Count() + " GlobalOutfkitKits and " + enumerable.Count() + " OutfitVariationCollections! Now auto-checking for reapply.");
			currentStatus++;
			break;
		}
		case 1:
		{
			long num = mem.readLong((outfitVariationCollectionCheckAddress + 24).ToString("X2") + "," + charsetType);
			long num2 = mem.readLong((outfitVariationCollectionCheckAddress + 24).ToString("X2") + ",0x28");
