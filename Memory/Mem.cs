...
public class Mem
{
    ...
    private Dictionary<string, CancellationTokenSource> FreezeTokenSrcs =
    ...
    )
    {
        if (Is64Bit || IntPtr.Size == 8)
        {
            ...
            UIntPtr result = Native_VirtualQueryEx(
                hProcess,
                lpAddress,
                out lpBuffer2,
                new UIntPtr((uint)Marshal.SizeOf(lpBuffer2))
            );
        ...
        UIntPtr result2 = Native_VirtualQueryEx(
            hProcess,
            lpAddress,
            out lpBuffer3,
            new UIntPtr((uint)Marshal.SizeOf(lpBuffer3))
        );
    ...
    public bool OpenProcess(int pid)
    {
        try
        {
            ...
            Is64Bit =
                ...
                && IsWow64Process(pHandle, out var lpSystemInfo)
                && !lpSystemInfo;
            return true;
        }
        catch
    ...
    public int readPByte(UIntPtr address, string code, string file = "")
    {
        ...
        if (
            ReadProcessMemory(
                pHandle,
                UIntPtr.Add(address, LoadIntCode(code, file)),
                array,
                (UIntPtr)1uL,
                IntPtr.Zero
            )
    ...
    public bool writeMemory(string code, string type, string write, string file = "")
    {
        ...
        switch (type)
        {
            ...
            case "bytes":
                ...
                {
                    string[] array2 = (
                        (!write.Contains(",")) ? write.Split(' ') : write.Split(',')
                    );
    ...
    public UIntPtr getCode(string name, string path = "", int size = 8)
    {
        ...
        if (text2.Contains(','))
        {
            ...
            if (text.Contains("base") || text.Contains("main"))
            {
                ReadProcessMemory(
                    pHandle,
                    (UIntPtr)(ulong)((int)(mainModule?.BaseAddress ?? IntPtr.Zero) + array3[0]),
                    array,
                    (UIntPtr)(ulong)size,
                    IntPtr.Zero
                );
            ...
            else if (!text.Contains("base") && !text.Contains("main") && text.Contains("+"))
            {
                ...
                ReadProcessMemory(
                    pHandle,
                    (UIntPtr)(ulong)((int)intPtr + array3[0]),
                    array,
                    (UIntPtr)(ulong)size,
                    IntPtr.Zero
                );
    ...
    public UIntPtr get64bitCode(string name, string path = "", int size = 16)
    {
        ...
        if (text2.Contains(','))
        {
            ...
            if (text.Contains("base") || text.Contains("main"))
            {
                ReadProcessMemory(
                    pHandle,
                    (UIntPtr)(ulong)((long)(mainModule?.BaseAddress ?? IntPtr.Zero) + array3[0]),
                    array,
                    (UIntPtr)(ulong)size,
                    IntPtr.Zero
                );
            ...
            else if (!text.Contains("base") && !text.Contains("main") && text.Contains("+"))
            {
                ...
                ReadProcessMemory(
                    pHandle,
                    (UIntPtr)(ulong)((long)intPtr + array3[0]),
                    array,
                    (UIntPtr)(ulong)size,
                    IntPtr.Zero
                );
    ...
    public unsafe void InjectDLL(string strDLLName)
    {
        ...
        if (theProc.Responding)
        {
            ...
            IntPtr intPtr = CreateRemoteThread(
                pHandle,
                (IntPtr)(void*)null,
                0u,
                procAddress,
    ...
    )
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            uIntPtr = VirtualAllocEx(
                pHandle,
                FindFreeBlockForRegion(uIntPtr2, (uint)newBytes.Length),
                (uint)size,
                12288u,
                64u
            );
        ...
        int value2 = (int)(
            (long)(ulong)code2
            + (long)array.Length
            - ((long)(ulong)uIntPtr + (long)newBytes.Length)
            - 5
        );
    ...
    private UIntPtr FindFreeBlockForRegion(UIntPtr baseAddress, uint size)
    {
        ...
        {
            if (
                (long)(ulong)uIntPtr > (long)(ulong)lpSystemInfo.maximumApplicationAddress
                || (long)(ulong)uIntPtr < (long)(ulong)lpSystemInfo.minimumApplicationAddress
            )
            {
                uIntPtr = lpSystemInfo.minimumApplicationAddress;
            }
            if (
                (long)(ulong)uIntPtr2 < (long)(ulong)lpSystemInfo.minimumApplicationAddress
                || (long)(ulong)uIntPtr2 > (long)(ulong)lpSystemInfo.maximumApplicationAddress
            )
            {
        ...
        while (VirtualQueryEx(pHandle, uIntPtr4, out lpBuffer).ToUInt64() != 0L)
        {
            ...
            {
                if (
                    (long)(ulong)lpBuffer.BaseAddress % (long)lpSystemInfo.allocationGranularity
                    > 0
                )
                {
                    zero = lpBuffer.BaseAddress;
                    int num = (int)(
                        lpSystemInfo.allocationGranularity
                        - (long)(ulong)zero % (long)lpSystemInfo.allocationGranularity
                    );
                    if (lpBuffer.RegionSize - num >= size)
                    {
                        ...
                        if ((long)(ulong)zero < (long)(ulong)baseAddress)
                        {
                            ...
                            zero = UIntPtr.Subtract(
                                zero,
                                (int)((long)(ulong)zero % (long)lpSystemInfo.allocationGranularity)
                            );
                        }
                        if (
                            Math.Abs((long)((ulong)zero - (ulong)baseAddress))
                            < Math.Abs((long)((ulong)uIntPtr3 - (ulong)baseAddress))
                        )
                        {
                            uIntPtr3 = zero;
                        }
            ...
            if (lpBuffer.RegionSize % lpSystemInfo.allocationGranularity > 0)
            {
                lpBuffer.RegionSize +=
                    lpSystemInfo.allocationGranularity
                    - lpBuffer.RegionSize % lpSystemInfo.allocationGranularity;
            }
            uIntPtr5 = uIntPtr4;
    ...
    )
    {
        return Task.Run(
            delegate
            {
                ...
                MEMORY_BASIC_INFORMATION lpBuffer = default(MEMORY_BASIC_INFORMATION);
                while (
                    VirtualQueryEx(pHandle, uIntPtr, out lpBuffer).ToUInt64() != 0L
                    && uIntPtr.ToUInt64() < (ulong)end
                    && (ulong)((long)uIntPtr.ToUInt64() + lpBuffer.RegionSize) > uIntPtr.ToUInt64()
                )
                {
                    ...
                    if (flag)
                    {
                        ...
                        bool flag3 =
                            (lpBuffer.Protect & 4) != 0
                            || (lpBuffer.Protect & 8) != 0
                            || (lpBuffer.Protect & 0x40) != 0
                            || (lpBuffer.Protect & 0x80) != 0;
                        bool flag4 =
                            (lpBuffer.Protect & 0x10) != 0
                            || (lpBuffer.Protect & 0x20) != 0
                            || (lpBuffer.Protect & 0x40) != 0
                            || (lpBuffer.Protect & 0x80) != 0;
                    ...
                    {
                        uIntPtr = new UIntPtr(
                            lpBuffer.BaseAddress.ToUInt64() + (ulong)lpBuffer.RegionSize
                        );
                    }
                    else
                    {
                        ...
                        if (list.Count > 0)
                        {
                            ...
                            if (
                                (long)(ulong)memoryRegionResult.RegionBase
                                    + memoryRegionResult.RegionSize
                                == (long)(ulong)lpBuffer.BaseAddress
                            )
                            {
                ...
                Parallel.ForEach(
                    list,
                    delegate(
                        MemoryRegionResult item2,
                        ParallelLoopState parallelLoopState,
                        long index
                    )
                    {
                ...
                return (from c in bagResult.ToList() orderby c select c).AsEnumerable();
            }
        );
    }
    ...
    private unsafe long[] CompareScan(MemoryRegionResult item, byte[] aobPattern, byte[] mask)
    {
        ...
        ReadProcessMemory(
            pHandle,
            item.CurrentBaseAddress,
            intPtr,
            (UIntPtr)(ulong)item.RegionSize,
        ...
        {
            num = FindPattern(
                (byte*)intPtr.ToPointer(),
                (int)lpNumberOfBytesRead,
                aobPattern,
                mask,
                num + aobPattern.Length
            );
            if (num >= 0)
            {
